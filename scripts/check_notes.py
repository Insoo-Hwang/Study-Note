...
검사 항목은 실제로 겪었던 문제에서 나왔다.
  ...
  - 같은 설명이 `이어서 더 물으면`과 `답변 구조`에 두 번 실려 읽기 피곤했던 문제
...
# `답변 구조`·`최종 체크리스트`는 더 두지 않는다 — 아래 check_note() 4)번 참고.
...
def check_interview_format(rel: str, text: str) -> None:
    ...
    #    `답변 구조`는 바로 위 `이어서 더 물으면`을, `최종 체크리스트`는 `기본 질문`을
    #    같은 내용으로 한 번 더 쓴 것이라 2026-08-11에 전 노트에서 제거했다.
    for dropped in ("답변 구조", "최종 체크리스트"):
        if re.search(rf"^#{{3,4}} {dropped}\s*$", text, flags=re.MULTILINE):
            err(rel, f"`{dropped}` 절이 다시 들어왔다 "
                     f"(본문·`이어서 더 물으면`과 같은 내용이라 두지 않는다)")
