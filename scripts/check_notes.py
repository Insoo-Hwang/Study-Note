...
검사 항목은 실제로 겪었던 문제에서 나왔다.
  ...
  - 본문을 통째로 다시 쓴 절이 `6. 면접 정리`에 셋이나 있어 읽기 피곤했던 문제
...
# 본문을 다시 쓴 절들은 더 두지 않는다 — 아래 check_interview_format() 4)번 참고.
...
def check_interview_format(rel: str, text: str) -> None:
    ...
    # 4) 걷어낸 절이 다시 들어오지 않았는지
    #    셋 다 본문을 한 번 더 쓴 절이라 2026-08-11에 전 노트에서 제거했다.
    #    수치·코드 토큰 기준 재진술률은 `이어서 더 물으면` 91.2%,
    #    `답변 구조`·`최종 체크리스트`는 바로 옆 절과 사실상 같은 내용이었다.
    for dropped in ("답변 구조", "최종 체크리스트", "이어서 더 물으면"):
        if re.search(rf"^#{{3,4}} {dropped}\s*$", text, flags=re.MULTILINE):
            err(rel, f"`{dropped}` 절이 다시 들어왔다 "
                     f"(본문에 이미 있는 내용이라 두지 않는다)")
