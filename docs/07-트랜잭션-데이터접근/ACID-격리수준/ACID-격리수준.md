...
```text
UPDATE stock SET qty = qty - 1 WHERE id = 1;   →  결과 98 (실측)
```
...
| 구분          | 내용                                       |
| ----------- | ---------------------------------------- |
| **장점**      | 부분 실패와 보상 코드가 사라지고, 장애가 나도 커밋된 것은 남는다. 동시성 정책을 선언 한 줄로 바꿀 수 있고, 묶어서 커밋하면 크게 빨라진다(실측 298배). |
...
| 문제 | 격리 수준으로? | 실제 해법 |
| --- | --- | --- |
| 재고가 마이너스가 된다 | **안 된다** (실측 확인) | `UPDATE ... WHERE qty > 0` 또는 락 |
...
6. **트랜잭션이 길면 무엇이 문제인가요?**
    ...
    * 핵심 키워드: 옛 버전 보관(실측 28.7배), 커넥션 점유, 락 유지, 롤백 비용 증가
...
격리 수준은 **어떤 이상 현상을 허용할지 고르는 4단계**입니다. H2로 커넥션 두 개를 열고 직접 확인해 보면, `READ UNCOMMITTED`는 커밋되지 않은 2000을 그대로 읽었고 `READ COMMITTED`부터 1000을 읽었습니다. 같은 행을 두 번 읽는 테스트에서는 `READ COMMITTED`가 1000 → 2000으로 값이 변했고 `REPEATABLE READ`부터 고정됐습니다. 그런데 **`REPEATABLE READ`에서 팬텀까지 막혔습니다.** 표준 표와 다른 결과인데, H2가 스냅숏 기반 MVCC이기 때문입니다. **MySQL InnoDB도 같은 이유로 RR에서 팬텀이 거의 안 보입니다.** 교과서의 표는 "이 수준에서 그 현상이 나도 규격 위반이 아니다"라는 뜻이지 실제 엔진 동작이 아닙니다.

**가장 중요한 실측 결과는 갱신 손실이었습니다.** 재고 100에서 두 트랜잭션이 각각 1씩 빼면 98이 나와야 하는데, `READ COMMITTED`·`REPEATABLE READ`·`SERIALIZABLE` **전부 99가 나왔습니다.** 애플리케이션이 100을 읽어서 99를 계산해 "99로 만들어라"라고 쓰기 때문입니다. DB 입장에서는 아무 모순이 없습니다. **격리 수준은 읽기가 무엇을 보는가를 정할 뿐, 읽은 값으로 무슨 계산을 하는가에는 관여하지 않습니다.** 같은 작업을 `UPDATE stock SET qty = qty - 1` 한 문장으로 바꾸면 정확히 98이 됩니다. 그래서 재고나 쿠폰 문제는 격리 수준이 아니라 **원자적 UPDATE나 락으로** 풀어야 합니다.

성능 쪽에서는 두 가지를 기억합니다. **첫째, 커밋 비용은 데이터 양이 아니라 커밋 횟수를 따라갑니다.** 2,000건을 건건이 커밋하면 2,772.9 ms, 트랜잭션 하나로 묶으면 9.3 ms로 **298배** 차이였습니다. 커밋마다 로그를 디스크에 밀어 넣기 때문이고, `fsync`를 끄면 초당 721건이 24,203건으로 **33.6배** 빨라졌습니다. MySQL의 `innodb_flush_log_at_trx_commit`이 이 손잡이입니다. **둘째, 긴 트랜잭션은 옛 버전을 붙잡습니다.** 읽기 트랜잭션 하나를 안 닫았더니 저장 공간이 **28.7배**가 됐습니다. InnoDB에서 `History list length`가 치솟는 현상이 이것입니다. 그래서 실무에서는 트랜잭션 안에 외부 API 호출을 넣지 않고, 배치는 1,000건 단위로 나눠 커밋합니다.

#### 답변 구조

1. **정의** — 트랜잭션은 전부 되거나 전혀 안 되는 작업 단위이고 ACID는 그 보장의 이름이다. A·C·D는 실패에 대한 보장, I는 동시 실행에 대한 보장이며 등급이 있는 것은 I 하나뿐이다
2. **내부 원리** — 원자성은 언두 로그를 거꾸로 적용해서, 내구성은 데이터보다 로그를 먼저 디스크에 쓰는 WAL로 달성한다. 격리성은 MVCC(스냅숏)나 락으로 구현하는데, 스냅숏을 문장마다 뜨면 `READ COMMITTED`, 트랜잭션 시작 시 한 번 뜨면 `REPEATABLE READ`다
3. **복잡도**
    * 격리 수준 × 이상 현상 (H2 실측): RU는 3현상 전부 발생, RC는 Dirty Read만 차단, RR부터 3현상 전부 차단
    * 갱신 손실: RC·RR·SERIALIZABLE 전부 발생 (98 기대, 99 발생). 원자적 `UPDATE`로 바꾸면 98
    * 커밋 횟수: 2,000건 건건이 2,772.9 ms vs 하나로 묶기 9.3 ms (298배)
    * 내구성: `fsync` FULL 721 tx/s vs OFF 24,203 tx/s (33.6배)
    * 긴 트랜잭션: 저장 공간 0.21 MB → 6.02 MB (28.7배)
    * 락 기반 격리: 읽기 트랜잭션이 쓰기를 3,348 ms 막고 실패시킴. MVCC는 17.2 ms에 성공
4. **장점** — 부분 실패와 보상 코드가 사라지고, 장애가 나도 커밋이 남으며, 중간 상태가 밖에 노출되지 않는다. 동시성 정책을 선언 한 줄로 바꿀 수 있고, 묶어서 커밋하면 298배 빨라진다
5. **단점** — 커밋마다 디스크를 기다리고, 길어지면 옛 버전과 커넥션과 락을 붙잡는다. 갱신 손실은 어떤 격리 수준으로도 못 막고, DB 경계 밖(외부 API·다른 DB)에는 롤백이 걸리지 않는다
6. **사용 기준** — 트랜잭션 안에서 같은 데이터를 두 번 이상 읽고 그 값이 같아야 하면 `REPEATABLE READ`, 아니면 `READ COMMITTED`로 충분하다. 실무에서 격리 수준을 직접 만지는 일은 드물고, 정확성이 필요한 지점은 락이나 원자적 UPDATE로 푼다. 외부 호출·파일·메일은 트랜잭션 밖으로 뺀다
7. **대안과 비교** — 재고·쿠폰의 정확성은 격리 수준이 아니라 `UPDATE ... WHERE qty > 0`, 낙관적 락, 비관적 락, 분산 락으로 푼다. 중복 요청은 유니크 제약과 멱등성 키로 막는다. 서버가 여러 대면 `synchronized`는 무력하지만 트랜잭션은 그대로 동작한다. DB를 넘는 정합성은 2PC 대신 Saga와 보상 트랜잭션을 쓴다
8. **실무 적용 사례** — 주문·결제에서 결제 API 호출을 트랜잭션 밖으로 빼고 상태를 `PENDING → PAID`로 나눠 관리하며 멱등성 키로 중복을 막는다. 배치는 1,000~5,000건 단위로 나눠 커밋한다. 조회는 `@Transactional(readOnly = true)`로 변경 감지를 끄고 복제본으로 보낸다. `information_schema.innodb_trx`와 `History list length`를 알람으로 걸어 안 닫힌 트랜잭션을 조기에 잡는다
...
* [ ] 트랜잭션이 "전부 또는 전무"인 이유를 언두 로그로 설명할 수 있다.
* [ ] ACID 중 **A·C·D는 실패, I는 동시성**에 대한 보장임을 구분할 수 있다.
* [ ] 일관성(C)을 **DB가 아니라 제약과 개발자가 지킨다**고 설명할 수 있다.
* [ ] Dirty / Non-repeatable / Phantom을 **커밋 여부 · 값 변화 · 행 수 변화**로 구분할 수 있다.
* [ ] 격리 수준 4단계와 각 단계가 막는 현상을 표로 그릴 수 있다.
* [ ] 표준의 "허용"과 엔진의 "실제 발생"이 다르다는 것을 설명할 수 있다.
* [ ] **MVCC 엔진의 `REPEATABLE READ`가 팬텀까지 막는 이유**를 스냅숏으로 설명할 수 있다.
* [ ] **갱신 손실이 어떤 격리 수준으로도 안 막힌다**는 것과 그 이유를 설명할 수 있다.
* [ ] 갱신 손실을 원자적 `UPDATE`로 해결하는 쿼리를 쓸 수 있다.
* [ ] MySQL의 기본 격리 수준이 다른 DB와 다르다는 것을 안다.
* [ ] 커밋 비용이 **데이터 양이 아니라 커밋 횟수**에 비례하는 이유를 WAL로 설명할 수 있다.
* [ ] `innodb_flush_log_at_trx_commit`의 세 값과 각각의 손실 범위를 안다.
* [ ] 긴 트랜잭션이 옛 버전·커넥션·락을 붙잡는다는 것을 수치로 설명할 수 있다.
* [ ] `@Transactional`의 롤백 규칙과 자기 호출 함정을 안다.
* [ ] 외부 API 호출을 트랜잭션 밖으로 빼야 하는 세 가지 이유를 댈 수 있다.
* [ ] 대량 배치를 몇 건 단위로 커밋할지 정하고 이유를 말할 수 있다.
