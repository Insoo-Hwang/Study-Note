...
보통은 옵티마이저가 알아서 고른다(자동 선택이 5.5 ms로 가장 빨랐다). **통계가 낡았을 때만 오판한다.**
...
1. 무한 스크롤이면 총 건수 자체가 필요 없다
     → LIMIT 21 로 21건을 요청해 "다음 페이지 있음"만 판단한다 (1.0 ms)
...
| **단점**      | 조인은 인덱스가 없으면 무너지고 드라이빙 테이블 오판에 취약하며(283배) 샤딩 환경에서는 쓸 수 없다. 커서 방식은 페이지 번호로 이동할 수 없고 정렬 키가 유일해야 한다. |
...
└─ 예
     ↓
   데이터가 얼마나 되는가?
   ├─ 수만 건 이하   → OFFSET 으로 충분하다 (OFFSET 10,000 까지 0.15 ms)
   └─ 수십만 건 이상
        ↓
      뒤 페이지까지 실제로 가는가?
...
| 최적화 여지 | 크다 | 작다 |
| `WHERE`에 오른쪽 조건 | 영향 없음 | **`INNER JOIN`으로 격하된다** |
...
```text
원인:  데이터가 늘어 옵티마이저가 드라이빙 테이블을 바꿨다 (283배 격차)
...
```text
1. WHERE 가 orders 를 하루치로 좁힌다        → 드라이빙 테이블이 작아진다 (24배)
2. 그 인덱스로 정렬까지 처리된다             → filesort 가 없다
3. LIMIT 20 에서 멈출 수 있다               → 드라이빙 테이블 컬럼 정렬 (1,197배)
