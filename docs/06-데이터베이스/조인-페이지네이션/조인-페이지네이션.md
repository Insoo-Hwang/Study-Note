...
| 키워드 | 연결되는 이유 |
| --- | --- |
| **인덱스와 실행 계획** | 조인 키 인덱스와 커버링이 이 노트의 전제다. 앞 노트다. |
| **대용량 데이터 분할** | 샤딩하면 조인과 페이지네이션이 통째로 달라진다. 다음 노트다. |
| **`EXPLAIN` 조인 순서 읽기** | 어느 테이블이 드라이빙인지 실행 계획에서 확인한다. |
| **JPA 영속성 컨텍스트** | N+1이 왜 생기는지의 근본이다. |
| **QueryDSL 동적 쿼리** | 검색 조건이 여럿인 목록에서 실무 표준이다. |

### 실무 확장

| 키워드 | 연결되는 이유 |
| --- | --- |
| **P6Spy · datasource-proxy** | 요청당 쿼리 수를 세어 N+1을 운영에서 감지한다. |
| **`PageableExecutionUtils`** | `COUNT` 쿼리를 필요할 때만 실행한다. |
| **DTO 프로젝션** | 엔티티 대신 필요한 컬럼만 조회해 조인 비용을 줄인다. |
| **`STRAIGHT_JOIN`과 옵티마이저 힌트** | 통계가 오판할 때의 최후 수단이다. |
| **GraphQL DataLoader** | N+1을 배치로 묶는 같은 아이디어의 다른 구현이다. |

### 심화 학습

| 키워드 | 연결되는 이유 |
| --- | --- |
| **해시 조인과 정렬 병합 조인** | 분석 쿼리에서 중첩 루프를 대체하는 알고리즘이다. |
| **Relay 커서 규격** | 커서 페이지네이션의 사실상 표준 인터페이스다. |
| **CQRS와 읽기 전용 모델** | 목록 조회를 조인 없이 만드는 근본적 접근이다. |
| **검색 엔진(Elasticsearch) 연동** | 복잡한 조건 + 정렬 + 페이징을 통째로 넘긴다. |
| **비정규화 설계 판단** | 조인을 줄이려고 중복을 감수하는 기준을 세운다. |
