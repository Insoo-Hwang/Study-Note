...
* **풀이 고갈되면 요청이 실패한다.** 풀 2개에 동시 10개를 던졌더니 **6건 성공, 4건이 `SQLTransientConnectionException`** 으로 실패했다(실측).
...
* **커넥션을 반납하지 않으면 풀이 완전히 막힌다.** 3개짜리 풀에서 3개를 빌리고 안 닫자 네 번째부터 아무도 못 쓰게 됐다(실측).
...
| 구분          | 내용                                       |
| ----------- | ---------------------------------------- |
| **장점**      | 커넥션 생성 비용(실측 0.5 ms/개)을 없애고 **DB 동시 접속에 상한을 두어 DB를 보호**한다. 커넥션 상태를 관측·검증할 수 있고, 타임아웃으로 빨리 실패시켜 장애 확산을 막는다. |
...
| **주의할 상황**  | 느리다고 **풀 크기부터 늘리는 것**(실측 20 이상은 효과 없음), 트랜잭션 안에서 외부 API를 호출하는 것, `try-with-resources`를 빠뜨리는 것. |
...
| ---------- | ------------------ | ----------------------- |
| **생성 비용**  | 없다 (재사용)           | 요청마다 발생 (실측 0.496 ms/개) |
...
| `PreparedStatement`는 성능 때문에 쓴다  | 성능(실측 4.9배)도 있지만 **SQL 인젝션 방어가 더 중요하다.** 실측에서 1,000명 vs 0명이었다.    |
...
```java
// ② IN 절로 묶기 — 페이지네이션과 함께 쓸 수 있다 (실측 1.2 ms)
...
8. **`PreparedStatement`를 쓰는 이유는 무엇인가요?**
    ...
    * 핵심 키워드: **SQL 인젝션 방어**(실측 1,000명 vs 0명), 파싱 재사용(실측 4.9배)
...
직접 재 봤을 때 가장 인상적이었던 건 **풀 크기를 늘려도 효과가 없는 지점이 있다**는 것이었습니다. 작업 200개를 처리하는 데 풀 1개면 1,419 ms, 5개면 319 ms로 좋아지다가 **20개에서 97 ms로 포화됐고 50개로 늘려도 98 ms**로 그대로였습니다. 그래서 "느리니까 풀을 늘리자"는 대부분 잘못된 처방이고, 먼저 **왜 커넥션을 오래 붙잡는지**를 봐야 합니다. 순서는 느린 쿼리와 N+1, 그다음 트랜잭션 범위, 그다음 외부 API 호출이고, 풀 크기는 마지막입니다.

풀 고갈도 재현해 봤습니다. 풀 2개에 400 ms짜리 작업 10개를 동시에 던지고 타임아웃을 1초로 뒀더니 **6건만 성공하고 4건이 `SQLTransientConnectionException`** 으로 실패했습니다. 메시지가 `Connection is not available, request timed out after 1005ms`인데, 운영에서 이 로그가 보이면 원인은 셋 중 하나입니다. 풀이 작거나, 오래 붙잡는 코드가 있거나, **반납을 안 하는 누수**입니다. 누수는 3개짜리 풀에서 3개를 빌리고 안 닫으니 네 번째부터 완전히 막히는 걸 확인했는데, 실무에서는 서서히 진행돼서 며칠 뒤에 터지고 재시작하면 잠시 괜찮아지기 때문에 원인을 놓치기 쉽습니다. 그래서 **`leakDetectionThreshold`를 켜 두는 게 중요**한데, 이걸 켜면 누수 지점 스택트레이스가 그대로 로그에 찍힙니다.

쿼리 쪽에서 효과가 가장 큰 건 **N+1 제거**였습니다. 사용자 1,000명의 주문을 읽을 때 N+1이 15.2 ms, `IN` 절로 묶으면 1.2 ms, 조인 한 번이면 0.7 ms로 **21.3배** 차이가 났습니다. 그런데 이건 **H2 인메모리라 네트워크 왕복이 0인 조건**이라, 원격 MySQL이면 쿼리마다 왕복이 붙어서 1,000배까지도 벌어집니다. 해결책 중에서는 조인이 가장 빠르지만 **일대다 조인은 행이 뻥튀기돼서 페이징이 깨지기** 때문에, 목록 조회에서는 `IN` 절 방식을 정석으로 씁니다.

한 가지 예상과 달랐던 결과도 있었는데, **배치 INSERT가 오히려 느렸습니다.** 한 건씩 넣으면서 한 번만 커밋한 게 6.8 ms, `addBatch`를 쓴 게 9.4 ms였습니다. 배치의 이득은 네트워크 왕복 횟수를 줄이는 건데 인메모리는 왕복이 없으니 버퍼 오버헤드만 남은 겁니다. **진짜 개선을 만든 건 배치가 아니라 커밋 횟수를 줄인 것**이었고, 매번 커밋 24.4 ms가 한 번 커밋 6.8 ms로 3.6배 좋아졌습니다. 원격 DB에서는 배치도 확실히 유리하니까, 환경을 밝히지 않은 벤치마크는 믿으면 안 된다는 교훈으로 남았습니다.

#### 답변 구조

1. **정의** — 커넥션 풀은 DB 커넥션을 미리 만들어 두고 애플리케이션에 빌려주고 돌려받는 저장소다. 생성 비용을 없애는 동시에 DB 동시 접속 수에 상한을 두어 DB를 보호한다
2. **내부 원리** — `getConnection()`은 유휴 커넥션이 있으면 즉시 주고, 없으면 `maximumPoolSize`까지 새로 만들며, 그것도 차면 반납을 기다리다 `connectionTimeout`을 넘기면 예외를 던진다. 받은 `Connection`은 프록시라 `close()`가 반납으로 동작한다
3. **복잡도**
    * 커넥션 생성 **0.496 ms/개** (H2 인메모리 기준, 네트워크 DB는 더 큼)
    * 풀 고갈: 풀 2 + 동시 10 → **성공 6 / 실패 4**, 최대 대기 811 ms
    * 풀 크기별: 1→1,419 ms, 5→319 ms, **20→97 ms, 50→98 ms(포화)**
    * 누수: 3개 미반납 시 **네 번째부터 완전 차단**
    * N+1 15.2 ms vs `IN` 1.2 ms(12.5배) vs 조인 0.7 ms(**21.3배**)
    * `PreparedStatement` 재사용 **4.9배**, 커밋 묶기 **3.6배**
    * SQL 인젝션: 문자열 조립 **1,000명 노출** vs `PreparedStatement` **0명**
4. **장점** — 생성 비용을 없애고 DB 동시 접속에 상한을 두어 애플리케이션을 늘려도 DB가 보호된다. 타임아웃으로 빨리 실패시켜 장애 확산을 막고, 활성·유휴·대기 수를 메트릭으로 관측할 수 있으며 죽은 커넥션을 걸러 준다
5. **단점** — 풀 크기가 동시성의 상한이라 오래 쥐는 코드 하나가 전체를 막고, 반납 누락이 서서히 서비스를 죽인다. 풀을 늘려도 효과가 없는 지점이 있고, `maxLifetime`이 DB 타임아웃보다 길면 끊긴 커넥션을 빌려줘 간헐적 오류가 난다
6. **사용 기준** — 풀은 10~20으로 작게 시작해 `connections_pending`을 보며 조정하고, 대기가 생기면 풀보다 먼저 쿼리·트랜잭션 범위·외부 호출을 본다. `connection-timeout`은 3초로 짧게, `maxLifetime`은 DB `wait_timeout`보다 짧게, `minimum-idle`은 최대와 같게 둔다. 목록 조회의 N+1은 `IN` 절로, 단건은 조인으로 푼다
7. **대안과 비교** — 매번 생성하는 방식은 DB 부하에 상한이 없어 사실상 운영 불가다. HikariCP는 Spring Boot 기본이고 가장 빠르므로 기본값을 그대로 쓴다. N+1 해결에서 조인은 가장 빠르지만 일대다에서 페이징이 깨지고, `IN` 절은 쿼리가 두 번이지만 페이징과 함께 쓸 수 있다
8. **실무 적용 사례** — `leakDetectionThreshold`를 5초로 켜 누수 지점을 스택트레이스로 잡고, `hikaricp_connections_pending`에 알람을 건다. 외부 API·메일은 `@TransactionalEventListener(AFTER_COMMIT)`으로 빼서 커넥션 점유 시간을 DB 작업으로만 한정한다. 목록 조회는 ID를 모아 `IN` 절로 한 번에 읽고, 대량 INSERT는 `batchUpdate` + `rewriteBatchedStatements=true`로 처리하며, 쿼리 수를 테스트로 고정해 N+1 회귀를 막는다
...
* [ ] 커넥션 생성이 비싼 이유를 **네 단계**로 설명할 수 있다.
* [ ] 풀의 가치가 속도뿐 아니라 **DB 동시 접속 상한**에 있다는 것을 안다.
* [ ] 풀에서 받은 `close()`가 **반납**이라는 것과 `try-with-resources`가 필수인 이유를 안다.
* [ ] 풀 고갈 시 나는 예외와 메시지를 알고 있다.
* [ ] **풀 크기를 늘려도 효과가 없는 지점이 있다**는 것을 실측 수치로 설명할 수 있다.
* [ ] 느릴 때 확인하는 순서(쿼리 → 트랜잭션 → 외부 호출 → 풀)를 말할 수 있다.
* [ ] `leakDetectionThreshold`로 누수를 잡는 방법을 안다.
* [ ] **커넥션 누수가 서서히 진행되어 위험하다**는 것을 설명할 수 있다.
* [ ] `maxLifetime`을 DB `wait_timeout`보다 짧게 잡는 이유를 안다.
* [ ] `minimum-idle`을 최대와 같게 두는 이유를 안다.
* [ ] `connection-timeout`을 짧게 두는 것이 왜 나은지 설명할 수 있다.
* [ ] **트랜잭션 안에 외부 API를 넣으면 안 되는 이유**를 커넥션 점유로 설명할 수 있다.
* [ ] N+1이 무엇이고 실측 격차가 얼마인지 말할 수 있다.
* [ ] N+1 해결책 세 가지와 **일대다 조인의 페이징 문제**를 안다.
* [ ] `PreparedStatement`의 **진짜 이유가 SQL 인젝션 방어**임을 실측으로 설명할 수 있다.
* [ ] **배치가 항상 빠른 것은 아니다**는 것과 그 이유를 안다.
* [ ] 풀 모니터링에서 **`connections_pending`** 을 봐야 하는 이유를 안다.
* [ ] 애플리케이션 인스턴스 수 × 풀 크기가 DB `max_connections`를 넘지 않아야 함을 안다.
