...
| **단점**      | 원본과 사본이 어긋난다. 갱신 순서를 잘못 잡으면 **영구 불일치가 100% 발생**했다(실측 400/400). 메모리 비용이 들고(TTL 포함 키당 117.2 B), 히트율이 떨어지면 캐시 왕복이 순수 낭비가 되며, 캐시가 비는 순간 원본이 몰매를 맞는다. |
| **적합한 상황**  | **읽기가 압도적으로 많고, 계산이 무겁고, 잠깐 낡아도 되는 데이터.** 상품 목록·통계·랭킹·설정값·외부 API 응답. 같은 답을 여러 사용자가 공유할수록 이득이 커진다. |
| **주의할 상황**  | **읽은 값으로 "된다/안 된다"를 판단해 쓰기까지 하는 로직**(재고·잔액·한도). 사용자마다 값이 달라 재사용되지 않는 데이터. 원본 조회가 이미 충분히 빠른 단건 PK 조회(실측 54.3 µs로 Redis 왕복보다 빨랐다). |
...
```text
1) 이 조회가 정말 느린가?
      아니오 → 캐시를 붙이지 않는다 (실측 단건 PK 54 µs는 캐시가 손해다)
      예 ↓
...
1. **캐시를 붙였는데 오히려 느려졌습니다. 왜일까요?**
    ...
    * 핵심 키워드: 원본이 이미 빠름(실측 단건 PK 54.3 µs < Redis 540 µs), 히트율이 낮음, 직렬화 비용
...
1. **정의** — 원본과 같은 답을 더 가까운 저장소에 하나 더 두어, 반복되는 조회·계산을 재사용하는 장치다. 얻는 것은 속도이고 치르는 값은 원본과 사본의 어긋남이다
2. **내부 원리** — 읽기는 Cache-Aside가 표준이다. 캐시를 먼저 보고 없으면 원본에서 읽어 TTL과 함께 넣는다. 쓰기는 캐시를 갱신하지 않고 삭제하는데, 삭제가 멱등이라 두 쓰기의 순서가 뒤집혀도 안전하기 때문이다. 그리고 **DB를 먼저 고친 뒤 캐시를 지운다.** 반대 순서는 삭제와 갱신 사이의 창에 읽기가 들어와 옛 값을 박제한다. TTL은 어긋남의 최대 지속 시간을 정하는 최종 안전망이고, 축출은 TTL과 달리 수명이 남아도 메모리 부족으로 밀어내는 별개의 장치다
3. **복잡도**
    * 조회 지연 — 로컬 캐시 0.149 µs / Redis 540 µs / DB 단건 PK 54.3 µs / DB 조인+집계 677.9 ms
...
4. **장점** — 무거운 계산을 한 번만 하고 재사용해 원본 부하를 히트율만큼 덜어낸다. 실측에서 677.9 ms 쿼리가 540 µs가 됐고 히트율 99%에서 평균 응답이 194배 개선됐다. TTL과 축출이 저장소에 내장돼 있고, 분산 캐시면 서버가 몇 대든 사본이 한 벌이다. Cache-Aside는 캐시가 죽어도 서비스가 계속 돌아간다
5. **단점** — 원본과 어긋나고, 순서를 잘못 잡으면 그 어긋남이 영구적이다(실측 100%). 미스일 때는 캐시 왕복이 순수 낭비이고 원본이 빠르면 아예 손해다(단건 PK 54.3 µs < Redis 540 µs). 메모리 비용이 들고 TTL을 걸면 42.4% 더 든다. 히트율이 조금만 떨어져도 응답이 급격히 나빠지며, 캐시가 죽으면 DB 부하가 히트율의 역수만큼 뛴다
6. **사용 기준** — 읽기가 압도적으로 많고, 계산이 무겁고, 여러 요청이 같은 답을 공유하며, 잠깐 낡아도 되는 데이터에 쓴다. 인덱스·쿼리 튜닝을 먼저 하고 그래도 비싸면 캐시한다. **읽은 값으로 된다/안 된다를 판단해 쓰기까지 하는 데이터(재고·잔액·한도)는 캐시하지 않는다.** TTL은 무효화 로직이 있으면 길게, 없으면 짧게 잡는다
7. **대안과 비교** — 읽기 복제본은 읽기 물량을, 요약 테이블은 집계 비용을 줄이므로 캐시와 대체재가 아니라 함께 쓴다. 로컬 캐시는 0.149 µs로 훨씬 빠르지만 서버마다 사본이 따로 놀아 무효화가 전파되지 않으므로, TTL을 10초쯤으로 짧게 잡아 어긋남의 상한을 정하고 L2로 Redis를 둔다. 쓰기 전략 중 Write-Back은 유실을 감수할 수 있는 카운터류에만 쓴다
8. **실무 적용 사례** — 쓰기는 `@TransactionalEventListener(AFTER_COMMIT)`에서 `DEL`을 호출해 커밋 뒤에만 무효화한다. `@Cacheable`은 자기 호출에서 동작하지 않으므로 클래스를 분리하고, TTL은 `RedisCacheManager` 설정에 캐시 이름별로 둔다. 캐시 접근은 `try-catch`로 감싸 캐시 장애가 서비스 장애가 되지 않게 하고, `keyspace_hits`/`keyspace_misses`로 히트율을 지표화해 90% 이하에 알람을 건다. `maxmemory`와 `allkeys-lru`를 명시하고, 없는 데이터는 짧은 TTL로 캐시해 캐시 관통을 막는다
...
* [ ] 캐시를 **"같은 답을 두 곳에 두는 것"** 으로 설명하고 거기서 정합성 문제가 나온다는 것을 안다.
* [ ] 캐시가 **손해인 경우**(원본이 캐시 왕복보다 빠른 경우)를 수치로 말할 수 있다.
* [ ] Cache-Aside의 읽기 흐름을 그릴 수 있고, 캐시 장애 시에도 동작하는 이유를 안다.
* [ ] 쓰기 전략 네 가지를 구분하고 실무에서 무효화가 가장 흔한 이유를 설명할 수 있다.
* [ ] **갱신이 아니라 삭제**를 하는 이유를 "멱등"으로 설명할 수 있다.
* [ ] **DB 갱신 → 캐시 삭제** 순서여야 하는 이유를 실측(400/400 vs 0/400)으로 설명할 수 있다.
* [ ] 그 순서도 완벽하지 않은 인터리빙(B')을 설명할 수 있다.
* [ ] `@Transactional` 안에서 캐시를 지우면 안 되는 이유를 안다.
* [ ] 히트율과 평균 응답의 관계가 **직선이 아닌 이유**를 설명할 수 있다.
* [ ] 히트율 99% → 95%가 왜 10배 악화인지 말할 수 있다.
* [ ] TTL의 역할이 **"어긋남의 최대 지속 시간"** 임을 안다.
* [ ] TTL의 메모리 비용(+42.4%)을 안다.
* [ ] 축출과 TTL의 차이를 안다.
* [ ] Redis 기본 `maxmemory-policy`가 `noeviction`이고 왜 바꿔야 하는지 안다.
* [ ] 캐시 관통이 무엇이고 어떻게 막는지 안다.
* [ ] 로컬 캐시의 무효화가 어려운 이유와 그 대응(짧은 TTL)을 안다.
* [ ] **캐시하면 안 되는 데이터**의 판단 기준을 한 문장으로 말할 수 있다.
