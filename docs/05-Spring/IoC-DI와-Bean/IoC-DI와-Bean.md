...
* **빈은 기본이 싱글톤**이다. 두 번 꺼내도 같은 객체다(실측 `==` 비교 `true`).
...
* **순환 참조는 주입 방식에 따라 다르게 나타난다.** 생성자 주입은 **기동 시점에 `UnsatisfiedDependencyException`으로 즉시 실패**하고, setter 주입은 **기동이 성공해 문제가 숨는다**(실측 확인).
* 빈 생명주기 순서는 **생성자 → 의존성 주입 → `Aware` → `InitializingBean` → `initMethod` → (종료 시) `DisposableBean`** 이다(실측 확인).
...
| **단점**      | **빈이 싱글톤이라 상태를 두면 조용히 깨진다**(실측 96,303/100,000). 실행 시점에 어떤 구현이 주입됐는지 코드만 봐서는 모르고, 기동 시점 오류 메시지가 길어 원인 찾기가 어렵다. |
...
| **프록시 경유 호출**   | 메서드 본문이 거의 없을 때 **16.8배**(실측). 다만 절대값은 호출당 약 31 ns |
...
| 빈은 요청마다 새로 만들어진다               | **기본은 싱글톤**이다. 두 번 꺼내도 같은 객체다(실측 `true`).                       |
...
| 순환 참조는 어떤 주입 방식이든 실패한다         | **생성자 주입만 실패한다.** setter 주입은 **기동에 성공해 문제가 숨는다**(실측).           |
...
| 프록시 때문에 성능이 크게 떨어진다            | 호출당 약 **31 ns**다(실측). DB 한 번 가는 비용의 0.001%도 안 된다.               |
| 생성자에서 초기화 작업을 해도 된다            | 생성자 시점에는 다른 빈이 준비 안 됐을 수 있다. **`@PostConstruct`** 에 둔다(실측 순서 참고). |
...
4. **빈의 기본 스코프는 무엇인가요?**
    ...
    * 핵심 키워드: **싱글톤**, 컨테이너당 하나, 두 번 꺼내도 같은 객체(실측 `true`)

5. **빈 생명주기를 설명해 주세요.**

    * 핵심 키워드: 생성자 → 의존성 주입 → `Aware` → `InitializingBean` → `initMethod` → `DisposableBean`(실측)
...
3. **순환 참조가 생기면 어떻게 되나요?**
    ...
    * 핵심 키워드: **생성자 주입은 기동 시점 `UnsatisfiedDependencyException`**, setter는 **기동 성공해 숨음**(실측)
...
11. **프록시 때문에 성능이 떨어지지 않나요?**
    ...
    * 핵심 키워드: 호출당 약 **31 ns**(실측 16.8배지만 절대값이 미미), DB 한 번 비용의 0.001% 미만
...
싱글톤 문제를 직접 재현해 봤습니다. 필드에 `count`를 둔 빈을 100스레드에서 1,000번씩, 총 10만 번 증가시켰더니 **96,303**이 나왔습니다. 3,697번이 조용히 사라진 건데 예외도 로그도 없습니다. 그래서 빈 필드에는 **기동 시점에 정해지고 안 바뀌는 협력 객체만 `final`로** 두고, 요청마다 달라지는 값은 매개변수로 넘깁니다. 메서드 지역 변수는 스레드마다 스택이 따로라 안전합니다.

주입은 생성자 주입을 씁니다. 이유가 네 가지인데, `final`로 선언할 수 있고, 의존성이 빠지면 기동 시점에 바로 실패하고, **스프링 없이 `new`로 단위 테스트가 되고**, 순환 참조를 기동 단계에서 잡아 줍니다. 마지막 게 실제로 확인해 보니 인상적이었는데, **생성자 주입은 `UnsatisfiedDependencyException`으로 애플리케이션이 아예 안 뜨는 반면 setter 주입은 기동에 성공**하더군요. `a.b.a == a`가 `true`로 순환이 그대로 남은 채 서비스가 시작됩니다. 그래서 setter 주입이 더 관대해 보이지만 실은 **문제를 숨기는 것**이고, 순환 참조는 대개 한쪽이 일을 너무 많이 한다는 SRP 위반 신호라 고치는 게 맞습니다. Spring Boot 2.6부터 기본 금지가 된 것도 같은 맥락입니다.

구현체가 여러 개일 때는 `@Qualifier`보다 **`List<PaymentGateway>`로 전부 주입받아 `Map`으로 만들어 두고 실행 시점에 키로 고르는 방식**을 선호합니다. 결제 수단을 추가할 때 `@Component`만 붙이면 되고 선택하는 코드는 안 고쳐도 되기 때문입니다.

성능은 거의 문제가 안 됩니다. 프록시 경유 호출이 직접 호출보다 실측 16.8배였는데, 이건 메서드가 거의 아무것도 안 할 때의 비율이고 **호출당으로 환산하면 약 31 나노초**입니다. DB를 한 번만 가도 수 밀리초니까 여기에 신경 쓸 일은 없습니다.

#### 답변 구조

1. **정의** — IoC는 객체의 생성과 연결을 컨테이너가 대신 하도록 주도권을 뒤집은 원칙이고, DI는 필요한 객체를 밖에서 넣어 주어 그 원칙을 구현하는 방법이다. 빈은 컨테이너가 만들고 생명주기를 관리하는 객체다
2. **내부 원리** — 컨테이너는 먼저 빈 정의를 읽고, 정의 후처리를 거친 뒤 싱글톤 인스턴스를 만들어 의존성을 주입한다. 그다음 `BeanPostProcessor`가 각 빈을 감싸는데 `@Transactional` 프록시가 이 단계에서 만들어진다. 생명주기 콜백 순서는 생성자 → 의존성 주입 → `Aware` → `InitializingBean` → `initMethod` → `DisposableBean`이다
3. **복잡도**
    * 빈은 싱글톤 — 두 번 조회 시 `==` 비교 **true**
    * 싱글톤 공유 상태: 100스레드 × 1,000회에서 100,000 기대 → **96,303**
    * 순환 참조: 생성자 주입 **`UnsatisfiedDependencyException`**(기동 실패) vs setter 주입 **기동 성공**(`a.b.a == a` true)
    * 프로퍼티 우선순위: `PropertySource` 순서상 앞선 것이 이김
    * 프록시 호출: 1,000만 회에 직접 19.9 ms vs 프록시 333.9 ms(**16.8배**, 호출당 약 31 ns)
4. **장점** — 구현체를 몰라도 되어 DIP를 자연히 지키게 되고, 가짜 구현을 끼워 스프링 없이 밀리초 단위 단위 테스트가 가능하다. 조립 코드가 설정 한곳에 모이고, 싱글톤 재사용으로 객체 생성 비용이 없으며, 빈이기 때문에 AOP를 끼울 자리가 생긴다
5. **단점** — 싱글톤 공유 상태가 예외 없이 조용히 깨지고, 실행 시점에 어떤 구현이 주입됐는지 코드로 알 수 없다. 기동 오류 메시지가 길고, `new`로 만든 객체는 빈이 아니라 AOP·주입이 전부 안 걸린다. 프로토타입은 소멸을 관리하지 않아 자원 누수가 날 수 있다
6. **사용 기준** — 주입은 거의 항상 생성자 주입을 쓰고 선택적 의존성만 setter를 쓰며 필드 주입은 쓰지 않는다. 스코프는 상태가 없으면 싱글톤(기본), 요청별 상태가 필요하면 매개변수 전달을 먼저 고려하고 그다음 `@RequestScope`를 본다. 구현체가 계속 늘어나는 곳은 `Map`/`List` 주입으로 받는다
7. **대안과 비교** — 생성자 주입은 불변과 조기 실패를 주고 setter 주입은 선택적 의존성에만 맞으며 필드 주입은 테스트를 막는다. Spring 싱글톤은 GoF 싱글톤과 달리 컨테이너 범위이고 바꿔 끼울 수 있어 테스트가 가능하다. `@Qualifier`는 명시적이지만 구현체 추가 시 쓰는 쪽을 고쳐야 하고 `Map` 주입은 그럴 필요가 없다
8. **실무 적용 사례** — 결제 게이트웨이를 `List` 주입으로 받아 `Map`으로 인덱싱해 결제 수단 추가 시 선택 코드를 고치지 않는다. 요청별 사용자 정보는 빈 필드가 아니라 매개변수로 넘기고, DB 연결 확인 같은 초기화는 생성자가 아니라 `@PostConstruct`에 둔다. 순환 참조가 생기면 `ApplicationEventPublisher`로 의존 방향을 한쪽으로 만든다
...
* [ ] IoC와 DI를 **원칙과 구현 방법**으로 구분해 설명할 수 있다.
* [ ] 빈이 무엇이고 **`new`로 만든 객체와 무엇이 다른지** 설명할 수 있다.
* [ ] 컨테이너 기동 5단계를 말할 수 있고, 프록시가 어느 단계에서 만들어지는지 안다.
* [ ] 빈 생명주기 콜백 순서를 순서대로 말할 수 있다.
* [ ] **빈이 싱글톤이라 스레드 안전하지 않다**는 것을 96,303이라는 수치와 함께 설명할 수 있다.
* [ ] 빈 필드에 둬도 되는 것(협력 객체)과 안 되는 것(요청별 상태)을 구분할 수 있다.
* [ ] 생성자 주입을 권장하는 이유 **네 가지**를 말할 수 있다.
* [ ] 필드 주입을 쓰지 않는 이유를 설명할 수 있다.
* [ ] **순환 참조가 주입 방식에 따라 다르게 나타난다**는 것을 실측 결과로 설명할 수 있다.
* [ ] setter 주입이 순환 참조에 "관대한" 것이 왜 단점인지 설명할 수 있다.
* [ ] 순환 참조 해결 방법을 세 가지 이상 말할 수 있다.
* [ ] 프로토타입 빈을 싱글톤에 주입하면 왜 의도대로 안 되는지 안다.
* [ ] 프로토타입 빈의 **소멸은 관리되지 않는다**는 것을 안다.
* [ ] 구현체가 여러 개일 때 `Map` 주입이 OCP에 더 가까운 이유를 설명할 수 있다.
* [ ] 프록시 호출 비용이 **실무에서 무시할 수준**임을 수치로 말할 수 있다.
* [ ] 초기화 로직을 생성자가 아니라 `@PostConstruct`에 두는 이유를 안다.
