...
* **인터페이스가 있으면 JDK 동적 프록시, 없으면 CGLIB** 프록시가 만들어진다(`$Proxy18` vs `$$EnhancerBySpringCGLIB$$`).
...
| 프록시 때문에 성능이 크게 나빠진다               | 호출당 약 **31 ns**다. 정작 비싼 것은 **트랜잭션이 커넥션을 붙잡는 시간**이다.         |
...
8. **프록시 때문에 성능이 나빠지지 않나요?**
    ...
    * 핵심 키워드: 호출당 약 **31 ns**(16.8배지만 절대값 미미), 정작 비싼 건 **커넥션 점유 시간**
