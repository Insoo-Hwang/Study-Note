...
`HashMap`은 키의 `hashCode`로 버킷 인덱스를 **계산**해서 저장합니다. 탐색이 아니라 계산이라서 데이터가 아무리 많아도 단건 조회가 평균 `O(1)`입니다. 다만 서로 다른 키가 같은 버킷으로 가는 해시 충돌이 반드시 발생하는데, Java는 체이닝으로 잇다가 한 버킷에 8개 이상 쌓이고 용량이 64 이상이면 트리로 바꿔서 최악을 `O(log n)`으로 방어합니다. 대신 **순서를 전혀 보장하지 않고, 범위 검색을 못 합니다.**

`TreeMap`은 레드-블랙 트리로 키를 항상 정렬된 상태로 유지합니다. 조회는 `O(log n)`으로 해시보다 느리지만, **범위 검색이 `O(log n + k)`이고 정렬 순회에 추가 비용이 들지 않습니다.** "90점 이상 상위 10명" 같은 조회에서 `HashMap`은 전체를 정렬해야 하지만 `TreeMap`은 시작점만 찾으면 됩니다. 균형 트리라 최악에도 `O(log n)`이 보장된다는 점도 차이입니다.

`LinkedHashMap`은 `HashMap`에 양방향 연결 리스트를 덧붙여 **삽입 순서나 접근 순서를 기억**합니다. 조회는 여전히 `O(1)`이고, `accessOrder`와 `removeEldestEntry`를 조합하면 LRU 캐시가 됩니다.

`Set`은 별도 구조가 아니라 **값을 키 자리에 넣은 Map**입니다. `HashSet`은 내부에 `HashMap`을 그대로 갖고 있습니다.

가장 중요한 주의점은 **`equals`와 `hashCode` 계약**입니다. `equals`만 재정의하면 다른 버킷을 보게 되어 값을 영원히 찾지 못하고, 키로 쓴 객체의 필드를 바꿔도 마찬가지입니다. **키는 반드시 불변이어야 합니다.**

#### 답변 구조

1. **정의** — Hash는 위치를 계산, Tree는 비교하며 내려감. Set은 값을 키 자리에 넣은 Map
2. **내부 원리** — `hashCode` → 보조 해시 → `(capacity-1) & hash` → 체이닝 → 8개 이상이면 트리화 / 레드-블랙 트리의 색 규칙과 회전
3. **복잡도**
    * `HashMap` 조회 평균 `O(1)`, 최악 `O(log n)` (Java 8 이상)
    * `TreeMap` 모든 연산 `O(log n)`, 평균과 최악이 같음
    * 범위 조회 — `HashMap` `O(n)` vs `TreeMap` `O(log n + k)`
    * 정렬 순회 — `HashMap` `O(n log n)` vs `TreeMap` `O(n)`
4. **장점** — Hash는 단건 조회가 가장 빠름. Tree는 정렬·범위·최솟값과 최악 보장
5. **단점** — Hash는 순서 없음·범위 불가·`hashCode` 의존·빈 버킷 메모리. Tree는 단건 조회가 느리고 비교 기준 필수
6. **사용 기준** — 순서가 필요 없으면 `HashMap`, 삽입 순서면 `LinkedHashMap`, 크기 순서·범위면 `TreeMap`
7. **대안과 비교** — DB의 B+Tree 인덱스와 Hash 인덱스가 같은 구분. 고정 데이터면 정렬된 배열 + 이진 탐색이 더 나음
8. **실무 적용 사례** — N+1 해소용 `HashMap`, 순서 있는 API 응답에 `LinkedHashMap`, 랭킹·구간 조회에 `TreeMap`, 동시 접근에 `ConcurrentHashMap`
...
| 키워드                      | 연결되는 이유                                     |
| ------------------------ | ------------------------------------------- |
| **equals와 hashCode 계약**  | Hash 계열의 정확성이 전적으로 여기 달려 있다.                |
| **선형 자료구조 비교**           | `List`에서 `Set`·`Map`으로 바꿔야 하는 시점을 판단할 수 있다. |
| **Heap과 PriorityQueue**  | 전체 정렬 없이 Top-K만 뽑는 또 다른 선택지를 알 수 있다.        |
| **Comparable과 Comparator** | Tree 계열의 정렬 기준을 정의하는 방법이다.                  |
| **Java Collection**      | 인터페이스와 구현체 계약, 불변 컬렉션을 함께 정리할 수 있다.         |

#### 실무 확장

| 키워드                  | 연결되는 이유                                    |
| -------------------- | ------------------------------------------ |
| **ConcurrentHashMap** | 동시 환경에서 Map을 안전하게 쓰는 방법과 복합 연산 API를 배운다.   |
| **DB 인덱스와 B+Tree**   | 애플리케이션의 Hash·Tree 선택이 DB 인덱스와 같은 구조임을 안다.  |
| **Redis 자료구조**       | `Hash`, `Set`, `Sorted Set`이 각각 무엇에 대응하는지 안다. |
| **캐시 전략과 LRU**       | `LinkedHashMap`의 `accessOrder`가 실제 캐시 설계로 확장된다. |
| **JPA 엔티티 동등성**      | 영속화 전후 ID 변화가 `Set`·`Map`에서 문제를 일으킨다.      |
| **분산 락과 멱등성**        | 애플리케이션 `Set`으로 막을 수 없는 중복을 다루는 방법이다.       |

#### 심화 학습

| 키워드                     | 연결되는 이유                                      |
| ----------------------- | -------------------------------------------- |
| **레드-블랙 트리의 회전**        | 삽입·삭제 시 균형이 어떻게 복구되는지 단계별로 이해할 수 있다.         |
| **스킵 리스트**              | Redis ZSET과 `ConcurrentSkipListMap`의 내부 구조다. |
| **해시 DoS 공격**           | 의도적으로 충돌을 유발하는 공격과 트리화가 왜 방어책인지 이해한다.        |
| **Consistent Hashing**  | 분산 캐시에서 노드 추가·제거 시 재배치를 최소화하는 기법이다.          |
| **Bloom Filter**        | 메모리를 극단적으로 줄인 확률적 포함 여부 판단 구조다.              |
| **B-Tree와 B+Tree**      | 디스크 환경에서 트리를 어떻게 바꾸는지 이해할 수 있다.              |

### 최종 체크리스트

* [ ] Hash 계열과 Tree 계열의 차이를 한 문장으로 설명할 수 있다
* [ ] `HashMap`의 인덱스 계산 3단계를 설명할 수 있다
* [ ] 해시 충돌이 왜 반드시 발생하는지, Java가 어떻게 처리하는지 설명할 수 있다
* [ ] `equals`와 `hashCode` 계약을 설명하고 위반 시 결과를 말할 수 있다
* [ ] 키가 불변이어야 하는 이유를 설명할 수 있다
* [ ] `TreeMap`이 최악에도 `O(log n)`인 이유를 설명할 수 있다
* [ ] 범위 검색에서 `TreeMap`이 압도적인 이유를 수치로 설명할 수 있다
* [ ] `LinkedHashMap`이 순서를 유지하는 방법과 LRU 캐시 구현을 설명할 수 있다
* [ ] `Set`이 내부적으로 Map임을 설명할 수 있다
* [ ] 요구사항에 따라 5가지 구현체 중 하나를 골라 이유를 댈 수 있다
* [ ] 동시 접근 시의 대안과 복합 연산의 위험을 설명할 수 있다
