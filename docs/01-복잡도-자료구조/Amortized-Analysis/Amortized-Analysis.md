...
| 구분          | 내용                                       |
| ----------- | ---------------------------------------- |
| **장점**      | 가끔 발생하는 비싼 연산을 전체 비용으로 평균 내어 반복 연산의 현실적인 장기 성능을 설명한다. 입력 확률을 가정하지 않는다. |
| **단점**      | **연산 한 번의 최악은 여전히 `O(n)`이다.** 지연에 민감한 실시간 처리에서는 튀는 구간이 그대로 문제가 된다. |
| **적합한 상황**  | `ArrayList` 추가, `HashMap` 삽입처럼 **연속으로 여러 번 호출되는** 연산의 비용을 설명할 때. |
| **주의할 상황**  | "평균 `O(1)`"과 "상각 `O(1)`"을 같은 말로 쓰는 것. 상각은 입력 분포 가정 없이 **모든 시퀀스에서** 보장된다. |
...
**적용 조건**은 셋이다 — 여러 연산을 하나의 연속된 과정으로 볼 수 있고, 대부분은 저렴한데 일부만 비싸며, 그 비싼 연산이 발생하는 조건(배열 리사이징 규칙 등)이 명확할 때다. 동적 배열·해시 테이블의 리사이징, 동적 버퍼 증가가 대표적이다.

**부적합한 상황**은 개별 요청의 최대 응답 시간을 반드시 보장해야 하거나, 비싼 연산의 발생 빈도를 제한할 수 없거나, GC·네트워크·DB 락 같은 시스템 병목을 분석할 때다. 이럴 때는 상각이 아니라 최악 시간 복잡도를 봐야 한다. **개별 요청의 지연이 중요한 시스템이라면 상각 복잡도만으로는 부족하고 최악 시간 복잡도를 함께 확인해야 한다.**
...
`개별 연산 비용` · `연산 시퀀스` · `전체 비용` · `상각 비용` · `최악 시간 복잡도` · `평균 시간 복잡도` · `동적 배열` · `크기 증가 정책` · `size` · `capacity`

### 이어서 볼 주제

* **[시간 복잡도와 공간 복잡도](../시간-공간-복잡도/시간-공간-복잡도.md)** — 상각 비용을 `O(1)`, `O(n)` 형태로 표현하는 기본 표기법.
* **[Java Collection](../../03-Java/Java-Collection/Java-Collection.md)** — `HashMap` 리사이징·`StringBuilder` 버퍼 확장에도 같은 상각 분석이 적용된다.
* **[JVM 메모리와 GC](../../03-Java/JVM-메모리-GC/JVM-메모리-GC.md)** — 배열 확장·복사가 실제 힙 할당과 GC에 미치는 영향.
* **대용량 데이터 분할** — 상각 `O(1)`이 전체 저장 비용 `O(n)`을 가려서는 안 되는 이유, 페이징·스트리밍 처리로 이어진다.
