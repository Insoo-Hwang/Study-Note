...
| 구분          | 내용                                       |
| ----------- | ---------------------------------------- |
| **장점**      | 최우선 원소 조회가 `O(1)`, 삽입·삭제가 `O(log n)`으로 **최악까지 보장**된다. 배열 기반이라 가볍고 캐시 친화적이다. |
| **단점**      | 임의 값 검색과 범위 조회를 못 한다. 불안정하며, **순회 순서가 정렬 순서가 아니다.** |
| **적합한 상황**  | 전체 정렬이 필요 없고 최댓값·최솟값만 반복해서 꺼낼 때. 상위 K개만 필요할 때. |
| **주의할 상황**  | `PriorityQueue`를 `for`로 순회하며 정렬을 기대하는 것. 크기 제한이 없어 무한정 커질 수 있다. |
...
* **[해시와 트리 비교](../해시-트리-비교/해시-트리-비교.md)** — 검색·범위 조회가 필요할 때 Heap 대신 무엇을 쓰는지.
* **[선형 자료구조 비교](../선형-자료구조-비교/선형-자료구조-비교.md)** — `PriorityQueue`가 왜 FIFO가 아닌지, `ArrayDeque`와의 차이.
* **[시간 복잡도와 공간 복잡도](../시간-공간-복잡도/시간-공간-복잡도.md)** — Top-K에서 `O(n log K)`와 `O(K)`가 중요한 이유.
