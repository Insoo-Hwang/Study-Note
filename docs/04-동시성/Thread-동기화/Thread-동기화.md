...
가시성 문제는 CPU 캐시 때문에 생깁니다. 각 코어가 자기 캐시의 복사본을 읽기 때문에, 한 스레드가 값을 바꿔도 다른 스레드가 못 볼 수 있습니다. 실제로 `volatile` 없는 `boolean` 플래그로 루프를 멈추려 하면 **`false`로 바꾼 뒤 3초가 지나도 워커 스레드가 `RUNNABLE` 상태로 계속 돌았습니다.** JIT가 반복문 밖으로 변수 읽기를 끌어내는 최적화까지 겹치기 때문입니다.

원자성 문제는 다릅니다. `count++`는 읽고·더하고·쓰는 세 단계라, 두 스레드가 같은 값을 읽으면 나중 쓰기가 앞 쓰기를 덮어씁니다. 100스레드가 1만 번씩 증가시킨 실측에서 기대값 100만에 대해 plain은 893,788이 나왔습니다. 여기서 중요한 것은 **`volatile`을 붙였더니 442,379로 오히려 더 많이 유실됐다는 점**입니다. 매번 메인 메모리를 오가느라 읽기와 쓰기 사이가 벌어져서 끼어들 창이 넓어졌기 때문입니다. `volatile`이 원자성과 무관하다는 것을 보여 주는 결과입니다.
