...
shutdownNow()
  새 작업 거부
  큐의 미실행 작업을 List 로 반환 (1개 반환)
  실행 중인 스레드에 interrupt 를 건다 (인터럽트 확인)
...
```text
스레드 직접 생성 (앞 노트)   개당 약 0.15 ms + 스택 약 1MB
풀에서 꺼내 쓰기                 큐 연산 한 번
```
...
| 단점 | 이유 |
| --- | --- |
| 실행기를 안 주면 공용 풀을 쓴다 | 병렬 수 5. 블로킹 작업을 넣으면 전역 영향. |
...
* **풀 크기를 무작정 크게** — CPU 바운드는 코어 수에서 포화된다.
...
| --- | --- | --- |
| 반환값 | 없음 | `Future` |
| 예외 발생 시 | `UncaughtExceptionHandler` 호출 (로그 남음) | **`Future`에 갇힌다** (무음) |
...
| 풀을 안 닫아도 GC가 정리해 준다 | 기본 스레드가 non-daemon이라 JVM이 종료되지 않는다. |
...
8. **`CompletableFuture`에 실행기를 안 넘기면 어떻게 되나요?**
    ...
    * 핵심 키워드: `ForkJoinPool.commonPool()`, 병렬 수 코어−1(5), JVM 전역 공유, 블로킹 금지
