...
**비밀번호 해시에서 핵심은 "느린 것이 미덕"이라는 역설입니다.** 일반적인 해시는 빠를수록 좋지만, 비밀번호에서는 그 속도를 공격자가 그대로 씁니다. 실제로 재보니 SHA-256 한 번이 **0.388 µs**, 단일 스레드로 **초당 258만 회**였고, 8자리 소문자+숫자 2조 8천억 가지를 **303.7시간**이면 전부 시험할 수 있었습니다. GPU를 쓰면 훨씬 빠르니 이건 낙관적인 수치입니다. PBKDF2로 반복을 걸면 1,000회에 1.6 ms, 10만 회에 140 ms, **60만 회에 864 ms**가 됐습니다.

**반복 횟수와 솔트는 막는 것이 다릅니다.** 반복 횟수는 **한 번의 시도를 비싸게** 만들어 무차별 대입을 막고, 솔트는 **같은 비밀번호가 다른 해시가 되게** 해서 미리 만든 표의 재사용을 막습니다. 그래서 솔트는 비밀이 아니어도 되고 해시와 함께 저장합니다. 자주 뭉뚱그려 설명되는데 역할이 분명히 다릅니다.

**느린 해시에는 대가도 있습니다.** 60만 회면 코어 하나로 **초당 1회**밖에 처리 못 해서, 로그인 엔드포인트가 DoS 표면이 됩니다. 그래서 반드시 **레이트 리밋**을 함께 겁니다.

**인가 쪽에서 가장 흔한 실수는 소유자 확인 누락입니다.** `/orders/{id}`에서 로그인 여부만 보고 소유자를 확인하지 않으면 번호만 바꿔서 남의 주문을 봅니다. **컴파일도 되고 테스트도 통과하고 오류도 안 나서** 조용히 취약해집니다. 저는 `findByIdAndUserId`처럼 **조회 조건에 소유자를 넣어** 빠뜨릴 수 없게 만들고, 없으면 403이 아니라 **404**를 줍니다. 403을 주면 "그 번호는 존재한다"는 정보가 새니까요.

**CORS는 가장 많이 오해받는 주제입니다.** CORS 오류는 서버가 요청을 거부한 게 아닙니다. **서버는 정상 처리했고 부수 효과까지 다 일어났는데**, 브라우저가 응답 헤더를 보고 스크립트에 안 넘긴 것뿐입니다. 그래서 `curl`로는 되는데 브라우저에서만 실패합니다. 그리고 **CORS는 제한을 푸는 규약이지 거는 규약이 아니라서**, 넓게 열수록 보안은 약해집니다. `Allow-Origin: *`와 `Allow-Credentials: true`를 같이 쓰려는 시도를 자주 보는데 브라우저가 거부하고, 억지로 우회하면 아무 사이트나 인증된 요청을 보낼 수 있게 됩니다.

**CSRF는 CORS와 방향이 반대입니다.** CORS가 "응답을 못 읽게"라면 CSRF는 **"요청은 이미 나갔다"** 는 문제입니다. 브라우저가 대상 도메인의 쿠키를 자동으로 붙이기 때문에, 사용자가 악성 사이트에 있어도 은행 요청이 정상 인증된 요청으로 처리됩니다. **응답을 못 읽어도 이체는 끝난 겁니다.** 방어 원리는 **"쿠키가 아닌 곳에서 증표를 받는 것"** 이고, CSRF 토큰이든 `SameSite`든 `Authorization` 헤더든 원리가 같습니다. 그래서 **쿠키로 인증하면 CSRF 방어가 필요하고, Bearer 헤더면 불필요**합니다.

**마지막으로 정직하게 말씀드릴 게 하나 있습니다.** 토큰 비교에 상수 시간 비교를 써야 한다는 걸 실측으로 보이려 했는데 **재현되지 않았습니다.** `String.equals`가 오히려 마지막 글자만 다를 때 빨랐고(4.99 ns vs 2.30 ns), 상수 시간이어야 할 `MessageDigest.isEqual`은 1.52배 차이가 났습니다. Java 9부터 `String.equals`가 **바이트를 벡터화해 8개씩 비교**해서 "첫 글자에서 멈춘다"는 전제가 깨졌고, JIT과 분기 예측이 나노초 차이를 덮어버린 겁니다. **실제 타이밍 공격은 네트워크 너머에서 수만 번을 통계적으로 누적**해 신호를 뽑기 때문에 로컬 한 번 측정으로는 안 보입니다. 상수 시간 비교는 비용이 거의 없으니 **증명이 어려워도 원칙적으로 쓰는 게 맞다**고 정리했습니다.

#### 답변 구조

1. **정의** — 인증은 요청자가 누구인지 확인하는 일이고 인가는 그 사람이 해당 자원에 무엇을 해도 되는지 확인하는 일이다. CORS는 브라우저의 동일 출처 제한을 서버가 응답 헤더로 풀어 주는 규약이고, CSRF는 쿠키 자동 전송을 악용해 사용자 의도 없이 요청을 발생시키는 공격이다
2. **내부 원리** — 인증 필터가 세션·토큰을 읽어 신원을 정하고(실패 401), 인가가 권한과 소유자를 검사한다(실패 403). 비밀번호는 반복 횟수로 계산을 일부러 느리게 만들어 공격자의 초당 시도를 줄이고, 솔트로 사용자마다 해시를 다르게 해 사전 계산 표를 무력화한다. CORS는 단순 요청이 아니면 `OPTIONS` 프리플라이트로 먼저 허용 여부를 묻고, CSRF 방어는 쿠키로 전달되지 않는 값을 추가로 요구해 성립한다
3. **복잡도**
    * SHA-256 단순 해시 **0.388 µs** — 단일 스레드 **초당 2,580,440회**
    * 8자리 소문자+숫자(2,821,109,907,456개) 전수 **303.7시간** (CPU 단일 스레드 기준, GPU는 훨씬 빠름)
    * PBKDF2-HMAC-SHA256 — 1,000회 1.6 ms / 10,000회 22.4 ms / 100,000회 140.2 ms / **600,000회 864.4 ms**
    * 60만 회는 SHA-256 대비 **223만 배** 느리다 → 공격자 초당 258만 회 → **초당 1회**
    * 상수 시간 비교 실측은 **재현 실패** (벡터화·JIT로 신호가 묻힘)
4. **장점** — 느린 해시는 DB 유출 이후에도 시간을 벌어 주는 유일한 방어이고, 솔트는 16 B로 레인보우 테이블을 무력화한다. 필터 체인으로 인증·인가를 분리하면 컨트롤러가 보안 코드를 갖지 않아도 되고, `SameSite=Lax`가 브라우저 기본값이 되면서 단순 CSRF는 설정 없이도 상당 부분 막힌다
5. **단점** — 느린 해시는 서버도 느려져 60만 회면 코어당 초당 1회라 로그인 DoS 표면이 되고, 반복 횟수는 하드웨어 발전에 맞춰 계속 올려야 한다. CORS는 JSON API에서 프리플라이트 왕복을 추가하며 넓게 열면 보호 효과가 사라진다. 인가 누락은 오류가 나지 않아 조용히 취약해지고, 타이밍 방어는 효과를 검증하기 어렵다
6. **사용 기준** — 비밀번호를 보관하면 느린 해시는 선택이 아니다. 새 프로젝트는 Argon2id, 무난하게는 BCrypt cost 12 이상, FIPS가 필요하면 PBKDF2 60만 회를 쓴다. 반복 횟수는 허용 가능한 로그인 지연과 초당 로그인 수 × CPU로 정하고 레이트 리밋을 함께 건다. CSRF 방어는 쿠키로 인증할 때만 켜고 Bearer 헤더면 끈다
7. **대안과 비교** — PBKDF2는 JDK 기본 제공이지만 GPU 저항이 약하고, BCrypt는 검증된 무난한 선택이며, Argon2id는 메모리까지 요구해 GPU 병렬화를 어렵게 만들어 새 프로젝트 1순위다. CORS와 CSRF는 이름이 비슷하지만 각각 응답 읽기와 요청 발생을 다루므로 대책이 서로를 대체하지 않는다. XSS와 SQL Injection은 모두 데이터가 해석기에서 코드로 읽히는 같은 구조라 방어도 같다
8. **실무 적용 사례** — `anyRequest().authenticated()`로 기본을 잠가 새 엔드포인트가 안전한 쪽으로 실패하게 하고, `sessionFixation().newSession()`으로 로그인 시 세션을 재발급한다. 비밀번호는 `DelegatingPasswordEncoder`로 저장해 알고리즘 식별자를 남기고 로그인 시 `upgradeEncoding`으로 점진 재해싱한다. 조회는 `findByIdAndUserId`처럼 소유자를 조건에 넣고 없으면 404를 반환한다. CORS는 출처를 구체적으로 지정하고 `maxAge`로 프리플라이트를 캐시하며, 로그인 실패 메시지를 하나로 통일해 계정 열거를 막고 로그인 엔드포인트에 레이트 리밋을 건다
...
* [ ] 인증과 인가를 구분하고 401·403을 올바로 쓸 수 있다.
* [ ] SHA-256으로 비밀번호를 저장하면 왜 부족한지 **수치로** 설명할 수 있다.
* [ ] 느린 해시가 방어가 되는 원리(공격자도 같은 비용)를 설명할 수 있다.
* [ ] 반복 횟수와 솔트가 **각각 다른 것을 막는다**는 것을 안다.
* [ ] 반복 횟수를 정하는 기준과 레이트 리밋이 왜 함께 필요한지 안다.
* [ ] CORS 오류가 **서버 거부가 아니라는 것**을 설명할 수 있다.
* [ ] 프리플라이트가 붙는 조건과 `Access-Control-Max-Age`의 역할을 안다.
* [ ] CORS와 CSRF가 각각 무엇을 막고 못 막는지 구분할 수 있다.
* [ ] CSRF 방어의 공통 원리를 "쿠키가 아닌 곳에서 증표를 받는다"로 말할 수 있다.
* [ ] `/orders/{id}`의 소유자 확인 누락이 왜 조용한 취약점인지 안다.
* [ ] 계정 열거를 막는 응답 설계를 할 수 있다.
* [ ] 상수 시간 비교를 쓰는 이유와, 그것을 로컬에서 증명하기 어려운 이유를 안다.
