...
| 구분          | 내용 |
| ----------- | -- |
| **장점**      | 세션은 서버가 상태를 쥐고 있어 **즉시 무효화**할 수 있고 쿠키가 작다(실측 **43 B**). JWT는 저장소 조회 없이 서명 검증만으로 인증이 끝나 **서버 간 공유가 필요 없고**(HS256 검증 실측 2.48 µs), 서비스가 여러 개여도 각자 검증한다. 쿠키 속성(`HttpOnly`·`Secure`·`SameSite`) 세 개로 주요 위협을 크게 줄인다. |
...
**먼저 쿠키는 인증 방식이 아니라 운반 수단**이라는 걸 짚고 갑니다. 세션 ID도 JWT도 쿠키에 담을 수 있어서, "쿠키냐 세션이냐"는 질문 자체가 성립하지 않습니다.

**JWT에서 가장 자주 오해받는 건 payload가 안전하다는 것입니다.** 실제로 서명 키 없이 디코딩해 봤는데 `{"sub":"1234567890","name":"Hong Gildong","role":"ROLE_USER",...}`가 **그대로 읽혔습니다.** Base64url은 암호화가 아니라 인코딩이라서요. **서명은 위조를 막을 뿐 내용을 감추지 않습니다.** 그래서 비밀번호나 주민번호를 담으면 안 되고, 감춰야 하면 JWE를 써야 합니다.

**JWT의 진짜 약점은 크기가 아니라 무효화입니다.** 서버가 상태를 안 갖는다는 건 로그아웃시킬 방법이 없다는 뜻이기도 합니다. 블랙리스트를 두면 되지만, 그러면 매 요청 저장소를 조회하게 되어 **JWT를 쓴 이유가 사라집니다.** 그래서 실무에서는 **짧은 Access Token과 서버에 저장하는 긴 Refresh Token**으로 나눕니다. 무효화가 필요한 지점을 Refresh 한 곳으로 몰아 놓는 설계인데, **Access 수명만큼의 빈틈은 남기 때문에** Access를 15분 같은 짧은 값으로 잡는 것 자체가 보안 설정이 됩니다.

**알고리즘 선택도 재봤습니다.** HS256 서명이 **2.48 µs(초당 40만 회)**, RS256 서명이 **2,258 µs(초당 442회)** 로 **909배** 차이였습니다. 그런데 RS256은 **검증이 96.2 µs로 서명의 23분의 1**입니다. 이 비대칭이 중요한 게, **서명은 인증 서버에서 로그인할 때만, 검증은 모든 서비스에서 매 요청** 일어나거든요. 비싼 쪽이 드문 쪽이라 감당이 됩니다. 그리고 HS256의 진짜 문제는 성능이 아니라 **검증하려면 서명 키가 필요해서, 그 키를 받은 서비스가 토큰을 위조할 수 있다**는 점입니다. 그래서 서비스가 여럿이면 RS256을 씁니다.

**세션 쪽에서 실무 사고는 대부분 서버를 늘릴 때 납니다.** 메모리 세션은 서버가 두 대가 되는 순간 깨지고, Sticky Session은 배포할 때마다 로그아웃됩니다. 그래서 Redis 같은 외부 저장소가 사실상 표준이고, Spring Session을 쓰면 설정 몇 줄로 옮길 수 있습니다. 그리고 **로그인 성공 시 세션 ID를 반드시 재발급**해야 합니다. 안 그러면 공격자가 미리 심어 둔 세션 ID가 그대로 로그인된 세션이 되는 **세션 고정 공격**에 당합니다.

**구현에서 조심할 게 하나 더 있는데 `alg: none` 공격입니다.** 공격자가 헤더의 `alg`를 `none`으로 바꾸고 서명을 지워서 보내는데, 라이브러리가 헤더의 `alg`를 그대로 믿으면 검증을 건너뜁니다. **검증할 알고리즘은 서버가 정해 놓고** 써야 합니다.

#### 답변 구조

1. **정의** — HTTP는 요청 간에 상태를 기억하지 않으므로 매 요청 신원을 다시 밝혀야 한다. 쿠키는 그 값을 운반하는 수단이고, 세션은 서버가 상태를 갖고 열쇠만 주는 방식, JWT는 서명된 정보를 클라이언트가 들고 다니는 방식이다
2. **내부 원리** — 세션은 로그인 시 저장소에 사용자 정보를 넣고 의미 없는 난수 ID를 쿠키로 준 뒤, 매 요청 그 ID로 저장소를 조회한다. JWT는 `header.payload.signature`를 Base64url로 잇고 비밀 키로 서명하며, 검증은 서명 재계산과 만료 확인으로 끝나 저장소를 보지 않는다. payload는 인코딩일 뿐이라 그대로 읽히고, 서명은 무결성만 보장한다. 무효화가 안 되므로 짧은 Access와 서버 저장 Refresh로 나눈다
3. **복잡도**
    * 쿠키 크기 — 세션 ID **43 B** vs JWT **221 B**(5.1배), 요청당 178 B 차이
    * JWT 구성 — header 36 B · payload 127 B · signature 43 B
    * 누적 전송량 — 요청 100만 건에 **169.8 MB** 추가
    * HS256 서명 **2.48 µs**(초당 402,606회)
    * RS256 서명 **2,258.3 µs**(초당 442회) → HS256 대비 **909배**
    * RS256 검증 **96.2 µs**(초당 10,400회) → RS256 서명의 1/23
4. **장점** — 세션은 저장소에서 지우는 것만으로 즉시 무효화되고 쿠키가 43 B로 작으며 훔쳐도 정보가 없다. JWT는 저장소 조회 없이 검증만으로 인증이 끝나 서비스가 여러 개여도 각자 처리할 수 있고, 인증 저장소가 죽어도 API가 계속 동작한다. 쿠키 속성 세 개로 주요 위협을 크게 줄일 수 있다
5. **단점** — 세션은 서버가 여러 대일 때 공유 저장소가 필요하고 그것이 단일 장애점이 된다. JWT는 만료 전 회수가 불가능하고, 크기가 5.1배이며 클레임을 더할수록 커진다. payload가 그대로 노출되고, 권한을 담으면 회수해도 토큰에 남는다. 블랙리스트로 무효화를 되찾으면 JWT의 장점이 사라진다
6. **사용 기준** — 즉시 로그아웃·강제 탈퇴가 중요하면 세션, 서비스가 나뉘어 인증 저장소 공유가 곤란하면 JWT를 쓴다. 브라우저 단일 서비스면 세션이 가장 단순하므로 기본값으로 삼는다. JWT를 쓰더라도 Access는 15~30분으로 짧게 잡고 Refresh는 서버에 저장해 취소 가능하게 둔다. 단일 서비스면 HS256, 검증자가 여럿이면 RS256이다
7. **대안과 비교** — 세션과 JWT는 상태를 누가 갖느냐로 갈리며 확장성과 무효화가 정반대로 트레이드오프된다. HS256은 빠르지만 검증자가 위조할 수 있어 신뢰 범위가 좁고, RS256은 공개 키만 배포하면 되어 여러 서비스에 적합하다. 담는 위치로는 쿠키가 CSRF에, 헤더+localStorage가 XSS에 약하므로 브라우저에서는 `HttpOnly` 쿠키와 CSRF 토큰 조합이 무난하다
8. **실무 적용 사례** — Spring Session으로 세션 저장소를 Redis로 옮기고 쿠키에 `HttpOnly`·`Secure`·`SameSite=Lax`를 설정한다. 로그인 성공 시 기존 세션을 `invalidate`하고 재발급해 세션 고정을 막는다. JWT는 `sub`·`role`·`exp`만 담고 개인정보를 넣지 않으며, 검증 시 서명·만료·허용 알고리즘을 모두 확인해 `alg: none`을 차단한다. 로그아웃은 Refresh를 저장소에서 지우고 쿠키를 같은 속성으로 덮어 만료시키며, Refresh 회전으로 재사용이 감지되면 그 사용자의 토큰을 전부 무효화한다
...
* [ ] 쿠키가 인증 방식이 아니라 **운반 수단**이라는 것을 설명할 수 있다.
* [ ] 세션과 JWT의 차이를 "상태를 누가 갖는가"로 정리할 수 있다.
* [ ] JWT의 세 조각과 실측 크기(43 B vs 221 B)를 말할 수 있다.
* [ ] payload가 **암호화가 아니라 인코딩**이라는 것을 실측으로 설명할 수 있다.
* [ ] JWT의 무효화 문제와 블랙리스트의 대가를 설명할 수 있다.
* [ ] Access/Refresh 분리가 무엇을 해결하고 무엇을 남기는지 안다.
* [ ] HS256과 RS256의 성능 차이(909배)와 **신뢰 범위** 차이를 구분할 수 있다.
* [ ] RS256에서 검증이 서명보다 싸다는 점이 왜 중요한지 안다.
* [ ] 서버가 여러 대일 때 세션 공유 방법 세 가지와 각각의 단점을 안다.
* [ ] 세션 고정 공격과 로그인 시 재발급의 이유를 설명할 수 있다.
* [ ] `HttpOnly`·`Secure`·`SameSite`가 각각 무엇을 막는지 말할 수 있다.
* [ ] `alg: none` 공격과 방어 방법을 안다.
