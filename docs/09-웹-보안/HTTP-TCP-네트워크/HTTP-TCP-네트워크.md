...
| **단점**      | 준비 비용은 **RTT에 비례**해서 서버를 아무리 빠르게 해도 줄지 않는다. 연결을 유지하면 서버가 소켓과 메모리를 계속 붙잡고, 헤더는 압축 없이 매 요청 반복된다(686 B). TLS는 CPU도 쓴다(루프백 핸드셰이크 16.31 ms). 타임아웃을 안 걸면 느린 상대 하나가 스레드를 영원히 점유한다. |
...
| 헤더 | 평문, 매번 반복 (686 B) | **HPACK 압축** | QPACK 압축 |
...
본문 전송이 전체의 21.2%였다. JSON은 압축이 잘 든다(다음 노트 기준 **21,996 B → 2,277 B, 9.7배**). 다만 **수백 바이트짜리 응답은 압축 비용이 이득보다 크므로** 임계값을 둔다.
