...
* **GC는 생각보다 자주, 그리고 짧게 돈다.** 500KB 객체 2,000개(약 1GB)를 만들었더니 **GC 8회, 누적 18 ms**였다(실측).
* **Stop-The-World(STW)** 는 GC가 도는 동안 애플리케이션 스레드를 멈추는 것이다. **응답 시간 문제의 주범**이다.
* **메모리 누수는 "쓰지 않지만 참조가 남아 있는" 객체**다. GC는 참조가 있으면 절대 치우지 않는다.
* `String` 리터럴은 **상수 풀에서 공유**되고 `new String()`은 힙에 새로 만든다(실측 `==` 비교로 확인).
* `Integer`는 **-128 ~ 127을 캐싱**한다. `127 == 127`은 `true`, `128 == 128`은 `false`다(실측).
...
| **속도**     | 매우 빠름             | 상대적으로 느림           |
| **크기**     | 작다 (기본 512KB~1MB) | 크다 (실측 4,082 MB)   |
| **넘치면**    | `StackOverflowError` (실측 23,404 프레임) | `OutOfMemoryError` |
| **동시성 문제** | **없다**            | **있다** (동기화 필요)    |
...
| `String`은 `==`로 비교해도 된다           | 리터럴끼리는 우연히 `true`지만 `new String()`은 `false`다(실측). 항상 `equals()`.    |
| 래퍼 타입도 `==`로 비교해도 된다              | **-128~127만 캐시**되어 `127==127`은 `true`, `128==128`은 `false`다(실측).    |
...
3. **객체를 많이 만들면 GC 때문에 느려지지 않나요?**
    ...
    * 핵심 키워드: **금방 죽으면 거의 공짜.** 1GB 생성에 GC 8회·**18 ms**(실측). 문제는 **오래 사는 객체**
...
11. **컨테이너에서 JVM 메모리 설정을 왜 조심해야 하나요?**
    ...
    * 핵심 키워드: 힙 기본값이 **물리 메모리 1/4**(실측 4,082 MB), 제한을 못 읽으면 **OOM Killer에게 조용히 죽음**

12. **`String a = "hello"`와 `new String("hello")`는 무엇이 다른가요?**

    * 핵심 키워드: 상수 풀 공유 vs 힙에 새 객체, `a == b` **true** / `a == c` **false**(실측), 항상 `equals()`

13. **`Integer`를 `==`로 비교하면 왜 위험한가요?**

    * 핵심 키워드: **-128~127만 캐시**, `127==127` true / `128==128` false(실측), 값이 커지면 운영에서만 터짐
...
직접 재 본 수치가 인상적이었습니다. 500KB 객체를 2,000개, 약 1GB를 만들어 버렸는데 **GC가 8번 돌았고 누적 시간이 18 ms**였습니다. 복사 방식 GC는 살아 있는 것만 옮기고 죽은 것은 아예 건드리지 않은 채 Eden을 통째로 비우기 때문입니다. 여기서 나오는 결론이 중요한데, **GC 비용은 "만든 양"이 아니라 "살아남은 양"에 비례**합니다. 그래서 튜닝의 방향도 객체를 적게 만드는 게 아니라 **Old로 승격되는 객체를 줄이는 것**입니다.

실무에서 문제가 되는 건 Minor GC가 아니라 **Full GC의 Stop-The-World**입니다. Old가 차면 힙 전체를 훑어야 해서 수백 밀리초에서 수 초까지 멈춥니다. "평소엔 50 ms인데 가끔 2초가 나온다"는 증상이 전형적인 신호입니다. 그래서 힙을 무작정 키우는 건 해법이 아니고 오히려 Full GC를 더 길게 만듭니다. JDK 9부터 기본인 **G1**은 힙을 Region으로 쪼개서 쓰레기가 많은 곳부터 치우고 목표 정지 시간을 지정할 수 있어서, 대부분의 서버는 G1 기본값을 그대로 씁니다.

**GC가 있어도 누수는 납니다.** 참조가 살아 있으면 GC는 절대 안 치우기 때문입니다. 제일 흔한 게 상한 없는 `static` 컬렉션이고, 그다음이 `ThreadLocal`을 스레드 풀에서 `remove()` 안 하는 경우입니다. 후자는 메모리보다 **보안이 더 위험**한데, 스레드가 재사용되면서 다음 요청이 이전 사용자의 컨텍스트를 볼 수 있습니다. 그래서 인터셉터의 `afterCompletion`처럼 예외가 나도 반드시 도는 자리에서 정리합니다.

운영 서버에는 **`-XX:+HeapDumpOnOutOfMemoryError`와 GC 로그를 미리 켜 둡니다.** 사고가 난 다음에는 켤 수 없기 때문입니다. `-Xms`와 `-Xmx`는 같게 고정하는데, 힙이 동적으로 늘어날 때 Full GC가 한 번 도는데 하필 트래픽이 올라가는 순간에 멈추기 때문입니다. 컨테이너에서는 절대값 대신 `MaxRAMPercentage`를 씁니다. 실측 환경에서 힙 기본값이 물리 메모리의 약 1/4인 4,082 MB로 잡혔는데, 컨테이너 제한을 못 읽으면 호스트 기준으로 계산해서 **JVM 로그도 없이 OOM Killer에게 죽습니다.**

#### 답변 구조

1. **정의** — JVM 메모리는 스레드별 영역(스택·PC 레지스터)과 공유 영역(힙·메타스페이스)으로 나뉘고, GC는 힙에서 더 이상 도달할 수 없는 객체를 회수해 메모리를 되돌려 주는 장치다
2. **내부 원리** — GC Root(스택의 지역 변수, `static` 변수 등)에서 참조를 따라가 닿지 않는 객체를 쓰레기로 판정한다. 힙은 Young(Eden + Survivor 2개)과 Old로 나뉘며, 객체는 Eden에서 태어나 Minor GC마다 Survivor를 오가며 나이를 먹고 임계값(기본 15)을 넘으면 Old로 승격된다. Survivor가 둘인 이유는 한쪽으로 복사하며 압축해 단편화를 없애기 위해서다
3. **복잡도**
    * 1GB 생성(500KB × 2,000) → GC **8회**, 누적 **18 ms**, 생존 10개
    * 최대 힙 기본값 **4,082 MB**(물리 메모리 약 1/4), 초기 힙 **256 MB**(약 1/64)
    * JDK 17 기본 GC = **G1** (`G1 Young/Old Generation`)
    * 스택 한계: **23,404 프레임**에서 `StackOverflowError` (고정값 아님)
    * `Integer` 객체 16바이트 vs `int` 4바이트 → 참조 포함 실질 5배
    * Minor GC 수 ms vs Full GC 수백 ms ~ 수 초
4. **장점** — 반납 실수(double free·dangling pointer)가 원천적으로 사라지고, 도달 가능성 판정이라 순환 참조도 회수된다. 압축된 Eden에서 포인터만 밀면 되어 객체 생성이 매우 싸고, 금방 죽는 객체는 거의 공짜이며 복사 과정에서 단편화가 자동 해소된다
5. **단점** — STW를 피할 수 없고 Full GC는 힙이 클수록 길어져 응답 시간이 튄다. 언제 도는지 제어할 수 없고 `System.gc()`는 보장이 없다. 참조가 남아 있으면 회수하지 않으므로 누수는 그대로 발생하며, 객체 헤더와 Survivor 예비 공간만큼 메모리를 더 쓴다
6. **사용 기준** — 응답 시간이 중요한 웹 서버는 G1 기본값을 그대로 쓰고, 배치처럼 처리량만 중요하면 Parallel, 힙이 수십 GB 이상이고 초저지연이 필요하면 ZGC를 본다. GC를 바꾸는 것은 측정으로 GC가 병목임을 확인한 다음이다
7. **대안과 비교** — 수동 관리는 예측 가능하지만 double free와 dangling pointer를 사람이 막아야 한다. 참조 카운팅은 즉시 회수되지만 순환 참조를 못 치운다. Parallel은 처리량이 가장 높고 G1은 정지 시간을 예측 가능하게 하며 ZGC는 1 ms 미만을 처리량과 맞바꾼다
8. **실무 적용 사례** — 운영 서버에 `HeapDumpOnOutOfMemoryError`와 GC 로그를 상시 켜 두고 `-Xms`와 `-Xmx`를 같게 고정한다. 컨테이너에서는 `MaxRAMPercentage`로 지정한다. `ThreadLocal`은 인터셉터 `afterCompletion`에서 반드시 `remove()`하고, 캐시는 Caffeine으로 크기·TTL 상한을 건다. 응답 시간이 주기적으로 튀면 GC 로그의 정지 시간부터 확인하고, OOM은 메시지 종류를 먼저 읽은 뒤 힙 덤프의 Path to GC Root로 참조를 붙잡고 있는 주체를 찾는다
...
* [ ] JVM 메모리를 **스레드별 영역과 공유 영역**으로 나눠 설명할 수 있다.
* [ ] 객체는 힙에, 참조 변수는 스택에 있다는 것을 그림으로 설명할 수 있다.
* [ ] GC가 **참조 카운트가 아니라 도달 가능성**으로 판단한다는 것과 그 이유(순환 참조)를 안다.
* [ ] GC Root가 되는 것을 세 가지 이상 말할 수 있다.
* [ ] 세대 구분의 근거가 **약한 세대 가설**임을 설명할 수 있다.
* [ ] 객체의 일생(Eden → Survivor → Old)을 순서대로 말할 수 있다.
* [ ] **Survivor가 두 개인 이유**를 단편화와 연결해 설명할 수 있다.
* [ ] Minor GC와 Full GC의 대상·빈도·시간 차이를 안다.
* [ ] **GC 비용이 "만든 양"이 아니라 "살아남은 양"에 비례**한다는 것을 18 ms 실측과 함께 설명할 수 있다.
* [ ] Stop-The-World가 왜 필요한지 설명할 수 있다.
* [ ] JDK 17 기본 GC가 **G1**임을 알고, G1이 Region 기반인 이유를 안다.
* [ ] Parallel · G1 · ZGC를 **처리량 vs 응답 시간**으로 구분해 고를 수 있다.
* [ ] **GC가 있어도 누수가 나는 이유**를 설명하고 대표 패턴 세 가지를 말할 수 있다.
* [ ] `ThreadLocal` 누수가 **메모리와 보안 양쪽 문제**임을 설명할 수 있다.
* [ ] `System.gc()`를 호출하면 안 되는 이유를 안다.
* [ ] **힙을 키우는 것이 해법이 아닌 이유**를 설명할 수 있다.
* [ ] 운영 서버에 넣을 JVM 옵션을 네 가지 이상 말할 수 있다.
* [ ] `-Xms`와 `-Xmx`를 같게 두는 이유를 설명할 수 있다.
* [ ] 컨테이너에서 힙 설정을 조심해야 하는 이유를 안다.
* [ ] OOM 메시지 종류를 구분하고 각각 무엇을 의심할지 안다.
* [ ] `String` 리터럴과 `new String()`의 차이를 `==` 결과로 설명할 수 있다.
* [ ] `Integer` 캐시 범위(**-128~127**)와 그것이 만드는 버그를 안다.
* [ ] PermGen이 메타스페이스로 바뀐 이유와 남은 주의점을 안다.
