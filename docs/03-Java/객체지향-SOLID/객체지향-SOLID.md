...
* **메서드는 오버라이딩되지만 필드는 오버라이딩되지 않는다.** `Parent p = new Child()`에서 `p.who()`는 `Child`를, `p.name`은 `Parent`를 준다(실측 확인).
* SOLID는 **SRP · OCP · LSP · ISP · DIP** 다섯 글자다. 외우는 것보다 **각각이 어떤 "코드 냄새"에 대한 처방인지** 아는 것이 중요하다.
* **LSP 위반은 문법 오류가 아니라 조용한 논리 오류다.** 정사각형을 직사각형의 자식으로 만들면 `5×4` 넓이가 **20이 아니라 16**이 된다(실측 확인).
...
* **생성자에서 오버라이딩된 메서드를 호출하면 자식 필드가 아직 `null`이다**(실측 확인). 상속이 만드는 대표적인 함정이다.
...
| **LSP 위반은 조용히 틀린다**    | 컴파일도 되고 예외도 없다. 값만 다르다(실측 20 vs 16).     |
...
| 필드도 오버라이딩된다                   | **필드는 가려질 뿐 오버라이딩되지 않는다.** 두 필드가 함께 존재하고 변수 타입으로 결정된다(실측).      |
| `static` 메서드도 오버라이딩된다         | 클래스에 속하므로 실행 시점에 고를 대상이 없다. 자식에 같은 시그니처를 쓰면 **숨김(hiding)** 이다.  |
| 컴파일되면 상속을 제대로 한 것이다           | **LSP 위반은 컴파일된다.** 정사각형-직사각형은 넓이가 20 대신 16이 나올 뿐 오류가 없다(실측).    |
...
1. **LSP를 위반한 예를 들어 주실 수 있나요?**
    ...
    * 핵심 키워드: 정사각형-직사각형, `setWidth(5) setHeight(4)`에서 **20 대신 16**(실측), 컴파일도 되고 예외도 없음
...
동작 원리에서 중요한 건 **결정 시점**입니다. `Parent p = new Child()`일 때 `p.who()`는 실행 시점에 실제 객체를 보고 `Child`를 실행하지만(동적 바인딩), `p.name`은 컴파일 시점의 변수 타입을 보고 `Parent`를 줍니다. 직접 확인해 봤는데 실제로 그렇게 나왔습니다. **필드는 오버라이딩되는 게 아니라 가려질 뿐**이고, 같은 이유로 `static` 메서드도 오버라이딩되지 않습니다.

실무에서 제일 조심하는 건 **상속**입니다. LSP 위반은 컴파일도 되고 예외도 안 나서 조용히 틀립니다. 정사각형을 직사각형의 자식으로 만들고 `setWidth(5); setHeight(4); area()`를 부르면 **20이 아니라 16**이 나옵니다. `HashSet`을 상속해 추가 횟수를 세는 코드도 `addAll`이 내부에서 `add`를 다시 호출해서 **3이 아니라 6**이 됩니다. 부모의 내부 구현에 자식이 묶인 건데, 이게 상속이 캡슐화를 깬다는 말의 실체입니다. 그래서 **합성을 기본으로 두고, is-a와 LSP를 둘 다 만족할 때만 상속**을 씁니다.

Spring은 사실상 DIP를 대신 지켜 주는 프레임워크입니다. 생성자 주입을 권장하는 이유도 원칙에서 나오는데, `final`로 선언할 수 있고 의존성 누락을 기동 시점에 발견하며 **스프링 없이도 `new`로 단위 테스트가 가능**하기 때문입니다. 구현체가 여러 개일 때는 `@Qualifier`보다 `Map<String, PaymentGateway>`로 전부 주입받아 실행 시점에 고르는 쪽을 선호합니다. 결제 수단을 추가할 때 `@Component`만 붙이면 되고 선택 코드는 안 고쳐도 되기 때문입니다.

다만 **SOLID를 많이 지킬수록 좋은 건 아니라고** 생각합니다. 구현체가 영원히 하나뿐인 인터페이스는 이득 없이 간접 계층만 늘립니다. 성능은 거의 문제가 안 됩니다. 구현체가 한둘이면 JIT가 인라인해서 가상 호출 비용이 사실상 사라지고, 진짜 비용은 **파일이 흩어져 흐름을 쫓기 어려워지는 인지 부하** 쪽입니다. 그래서 "세 번째 같은 것이 나타나면 그때 뽑는다"를 기준으로 씁니다.

#### 답변 구조

1. **정의** — 객체지향은 변경의 파급 범위를 좁히기 위해 데이터와 로직을 객체로 묶고 다형성으로 갈아 끼우게 만드는 설계 방식이다. SOLID는 그 방법을 다섯 원칙으로 정리한 것이다
2. **내부 원리** — 부모 타입 변수에 자식 객체를 담으면, 메서드는 실행 시점에 실제 객체를 보고 결정되고(동적 바인딩) 필드는 컴파일 시점의 변수 타입으로 결정된다(정적 바인딩). 그래서 호출부를 고치지 않고 구현만 바꿔 끼울 수 있다
3. **복잡도**
    * 필드 숨김: `p.who()`는 `Child`, `p.name`은 `Parent` (실측)
    * LSP 위반: 정사각형 상속 시 `5×4` 넓이가 20이 아닌 **16** (실측)
    * 상속의 캡슐화 파괴: `HashSet` 상속 카운터가 3이 아닌 **6**
    * 생성자 함정: 부모 생성자에서 오버라이딩 메서드 호출 시 자식 필드가 **`null`** (실측)
    * `equals` 대칭성: `pt.equals(cp)=true` / `cp.equals(pt)=false` (실측)
    * 가상 호출 비용: 구현체 1~2개면 JIT 인라인으로 사실상 0
4. **장점** — 기능 추가가 기존 코드 수정 없이 클래스 추가로 끝나고, 가짜 구현체를 끼울 수 있어 테스트가 쉬워지며, 인터페이스가 방화벽이 되어 변경이 밖으로 새지 않는다. 인터페이스만 합의하면 병렬 개발도 가능하다
5. **단점** — 파일과 간접 계층이 늘어 흐름 추적이 어렵고, 실행 시점에 어느 구현이 도는지 코드만으로는 모른다. 잘못 뽑은 추상화는 되돌리기 비싸다. 상속은 부모 변경에 자식이 끌려가고 LSP 위반은 예외 없이 조용히 틀린다
6. **사용 기준** — 같은 종류가 앞으로 늘어날 것이 확실할 때(결제 수단·할인 정책·알림 채널) 인터페이스로 뽑는다. "세 번째가 나타나면 뽑는다"를 기준으로 하고, 구현체가 하나뿐이면 만들지 않는다. 상속은 is-a와 LSP를 둘 다 만족할 때만 쓰고 그 외에는 합성을 쓴다
7. **대안과 비교** — 절차지향은 흐름이 위에서 아래로 읽히지만 분기가 쌓이면 손댈 수 없어진다. 상속은 간결하지만 결합이 강하고 실행 시점 교체가 안 되며, 합성은 위임 코드를 써야 하지만 유연하고 캡슐화가 유지된다. 인터페이스는 다중 구현이 되고 추상 클래스는 상태를 가질 수 있다
8. **실무 적용 사례** — Spring 생성자 주입으로 DIP를 강제하고, `@Transactional`은 프록시(다형성)로 원본을 안 고치고 트랜잭션을 붙인다. 결제 게이트웨이는 `Map<String, PaymentGateway>`로 전부 주입받아 실행 시점에 골라 결제 수단 추가 시 선택 코드를 고치지 않는다. 도메인 객체는 프레임워크를 모르게 유지해 스프링 없는 단위 테스트를 가능하게 한다
...
* [ ] 객체지향의 목적이 **"현실 모사"가 아니라 "변경 범위 축소"** 임을 설명할 수 있다.
* [ ] 네 기둥 중 **다형성이 실제 이득을 만드는 지점**이라고 설명할 수 있다.
* [ ] 다형성이 성립하는 세 조건을 말할 수 있고, **부모 타입으로 참조해야 의미가 있다**는 것을 안다.
* [ ] `Parent p = new Child()`에서 메서드와 필드의 결과가 왜 다른지 **결정 시점**으로 설명할 수 있다.
* [ ] 필드는 오버라이딩이 아니라 **숨김**이고 두 필드가 함께 존재한다는 것을 안다.
* [ ] `static` 메서드가 오버라이딩되지 않는 이유를 설명할 수 있다.
* [ ] SOLID 다섯 가지를 **각각 어떤 코드 냄새에 대한 처방인지**로 설명할 수 있다.
* [ ] OCP를 "분기문을 다형성으로 바꾸는 것"으로 코드와 함께 설명할 수 있다.
* [ ] LSP 위반 예시를 **20 vs 16**이라는 수치와 함께 들 수 있다.
* [ ] LSP 위반이 **컴파일도 되고 예외도 없이 조용히 틀린다**는 것을 안다.
* [ ] 상속이 캡슐화를 깨는 이유를 `HashSet` 카운터 예제(3 vs 6)로 설명할 수 있다.
* [ ] 생성자에서 오버라이딩 메서드를 부르면 안 되는 이유를 **초기화 순서**로 설명할 수 있다.
* [ ] 상속과 합성의 선택 기준을 **is-a + LSP**로 말할 수 있다.
* [ ] 인터페이스와 추상 클래스의 결정적 차이가 **상태를 가질 수 있는지**임을 안다.
* [ ] DIP와 DI의 관계를 **원칙과 수단**으로 구분해 설명할 수 있다.
* [ ] Spring 생성자 주입을 권장하는 이유를 네 가지 이상 댈 수 있다.
* [ ] **추상화를 하지 말아야 할 때**를 말할 수 있다 (구현체가 하나뿐일 때).
* [ ] 다형성의 진짜 비용이 성능이 아니라 **인지 부하**임을 설명할 수 있다.
