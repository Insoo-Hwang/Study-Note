...
| 키워드                 | 연결되는 이유                                       |
| ------------------- | --------------------------------------------- |
| **Java Collection** | 각 구현체의 내부 구조를 알면 선택 기준의 근거가 명확해진다.            |
| **equals와 hashCode** | `Set`·`Map` 선택이 이 두 메서드의 구현 품질에 달려 있다.        |
| **선형 자료구조 비교**      | `List`·`Queue`·`Deque` 계열의 트레이드오프를 더 깊이 이해한다. |
| **해시와 트리 비교**       | `Hash~`와 `Tree~` 중 무엇을 고를지 자료구조 관점에서 본다.      |
| **Heap과 PriorityQueue** | 우선순위가 필요한 상황의 자료구조를 이해한다.                     |

### 실무 확장

| 키워드                     | 연결되는 이유                                    |
| ----------------------- | ------------------------------------------ |
| **Concurrent Collection** | 동시 접근이 필요할 때의 선택지를 정확히 안다.                 |
| **Stream API**          | 선택한 컬렉션을 선언적으로 가공하는 표준 방식이다.               |
| **Caffeine·Guava Cache** | 상한이 있는 로컬 캐시를 제대로 구성하는 방법이다.               |
| **JPA 컬렉션 매핑**          | `List`와 `Set` 선택이 지연 로딩·N+1과 어떻게 얽히는지 안다.  |
| **Redis 자료구조**          | 분산 환경에서 로컬 컬렉션을 대체하는 선택지를 비교한다.            |

### 심화 학습

| 키워드                     | 연결되는 이유                                     |
| ----------------------- | ------------------------------------------- |
| **JMH 벤치마크**            | 컬렉션 성능 주장을 신뢰할 수 있게 측정하는 표준 도구다.            |
| **캐시 지역성과 메모리 계층**      | 같은 `O(n)`에서 성능이 갈리는 이유를 설명할 수 있다.           |
| **오토박싱과 기본형 컬렉션**       | Eclipse Collections·fastutil 같은 대안을 알 수 있다. |
| **Java 21 Sequenced Collections** | 순서 개념이 표준 인터페이스로 어떻게 정리됐는지 본다.              |
| **Backpressure와 유계 큐**  | 큐 크기 선택이 시스템 안정성에 미치는 영향을 배운다.              |
